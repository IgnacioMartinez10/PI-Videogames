...
const getVideogameByNameController = async (name) => {
    try {
        ...
        const apiResponse = await axios.get(`https://api.rawg.io/api/games?search=${game}?key=${API_KEY}`);
        const apiGames = apiResponse.data.filter((game) => game.name.toLowerCase().includes(name.toLowerCase())
        );
